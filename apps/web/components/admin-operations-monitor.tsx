    ...
    <section className="surface-panel p-6">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        {metrics.map((metric) => {
          ...
          return (
            <div className="rounded-[24px] border border-brand-line bg-white px-4 py-4" key={metric.id}>
              <span className={cn("inline-flex rounded-full px-3 py-1 text-xs font-semibold", styles.badge)}>
                {metric.label}
              </span>
              <p className={cn("mt-4 font-mono text-3xl font-semibold", styles.value)}>{metric.value}</p>
...
function taskStatusMeta(status: string) {
  ...
  return { label: status || "未知状态", className: "bg-slate-100 text-slate-700" };
}
      ...
      <section className="surface-panel overflow-hidden p-0">
        <div className="grid gap-0 xl:grid-cols-[1.1fr,0.9fr]">
          <div className="bg-[radial-gradient(circle_at_top_left,rgba(5,150,105,0.16),transparent_48%),linear-gradient(180deg,rgba(236,253,245,0.95)_0%,rgba(255,255,255,0.98)_100%)] px-6 py-7 sm:px-8">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <h3 className="mt-3 text-3xl font-semibold tracking-tight text-slate-900">业务运营控制台</h3>
              ...
              <button
                className="inline-flex items-center gap-2 rounded-2xl border border-brand-line bg-white/90 px-4 py-3 text-sm font-semibold text-slate-700 transition hover:border-emerald-200 hover:text-brand-emerald disabled:cursor-not-allowed disabled:opacity-60"
          ...
          <div className="bg-white px-6 py-7 sm:px-8">
            ...
            <h4 className="mt-3 text-2xl font-semibold text-slate-900">先看这四个总览指标</h4>
            <p className="mt-3 text-sm leading-6 text-slate-600">
              用任务量、活跃量、风险数和代理可用率判断当前是否适合继续放量。
            </p>

            {loading ? (
              <p className="mt-6 rounded-[24px] bg-stone-50 px-4 py-5 text-sm text-slate-500">正在加载业务监控...</p>
            ) : (
        ...
        <>
          <div className="grid gap-6 xl:grid-cols-[1.05fr,0.95fr]">
            <section className="surface-panel p-6">
              ...
              <h4 className="mt-3 text-2xl font-semibold text-slate-900">先处理这些风险</h4>
              <p className="mt-3 text-sm leading-6 text-slate-600">
              ...
              <div className="mt-5 space-y-4">
                {consoleData.risks.length ? (
                  consoleData.risks.map((risk) => (
                    <Link
                      className="group block rounded-[24px] border border-brand-line bg-white px-4 py-4 transition hover:-translate-y-0.5 hover:border-emerald-200 hover:shadow-editorial motion-reduce:transform-none"
                      href={risk.href}
                      key={risk.id}
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-start justify-between gap-3">
                            ...
                            <ArrowRight className="mt-0.5 h-4 w-4 flex-shrink-0 text-slate-400 transition group-hover:text-brand-emerald" />
                          </div>
                  ...
                  <div className="rounded-[24px] border border-brand-line bg-brand-mist/60 px-4 py-5">
                    <p className="text-sm font-semibold text-brand-emerald">当前没有明显风险</p>
            ...
            <div className="space-y-6">
              ...
              <section className="surface-panel p-6">
                ...
                <h4 className="mt-3 text-2xl font-semibold text-slate-900">代理池覆盖</h4>
                <p className="mt-3 text-sm leading-6 text-slate-600">
                ...
                <div className="mt-5 flex flex-wrap gap-2">
                  {consoleData.topCountries.length ? (
                      ...
                      <span
                        className={cn(
                          ...
                          country.tone === "emerald"
                            ? "border-emerald-200 bg-brand-mist text-brand-emerald"
                            : country.tone === "amber"
                    ...
                    <span className="rounded-full border border-brand-line bg-white px-3 py-1 text-xs text-slate-500">
                      暂无国家覆盖数据
                    </span>
                  )}
                </div>
          ...
          <div className="grid gap-6 xl:grid-cols-[1.05fr,0.95fr]">
            <section className="surface-panel p-6">
              ...
              <h4 className="mt-3 text-2xl font-semibold text-slate-900">补点击观察名单</h4>
              <p className="mt-3 text-sm leading-6 text-slate-600">
              ...
              <div className="mt-5 space-y-4">
                {consoleData.watchTasks.length ? (
                  consoleData.watchTasks.map((task) => {
                    ...
                    return (
                      <div className="rounded-[24px] border border-brand-line bg-white px-4 py-4" key={task.id}>
                  ...
                  <p className="rounded-[24px] border border-brand-line bg-white px-4 py-5 text-sm text-slate-500">
                    暂无需要关注的补点击任务。
                  </p>
                )}
              </div>
            ...
            <div className="space-y-6">
              <section className="surface-panel p-6">
                ...
                <h4 className="mt-3 text-2xl font-semibold text-slate-900">账号代理覆盖</h4>
                <p className="mt-3 text-sm leading-6 text-slate-600">
                ...
                <div className="mt-5 space-y-4">
                  {consoleData.proxyUsers.length ? (
                    consoleData.proxyUsers.slice(0, 6).map((user) => (
                      <div className="rounded-[24px] border border-brand-line bg-white px-4 py-4" key={user.userId}>
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <div className="flex flex-wrap items-center gap-2">
                            ...
                            <p className="mt-2 text-sm leading-6 text-slate-600">{user.note}</p>
                          </div>
                          <div className="text-sm text-slate-500 sm:text-right">
                            <p>
              ...
              <section className="surface-panel p-6">
                ...
                <h4 className="mt-3 text-2xl font-semibold text-slate-900">最近安全与配置操作</h4>
                <p className="mt-3 text-sm leading-6 text-slate-600">
                ...
                <div className="mt-5 space-y-4">
                  {consoleData.auditHighlights.length ? (
                    consoleData.auditHighlights.slice(0, 6).map((item) => (
                      <div className="rounded-[24px] border border-brand-line bg-white px-4 py-4" key={item.id}>
                        <div className="flex items-start gap-3">
                          <span
                            className={cn(
                              "mt-0.5 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-2xl",
